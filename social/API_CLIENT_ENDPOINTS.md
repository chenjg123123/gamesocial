...
### tournament-status-enum（约定）
赛事状态 `status` 建议使用以下枚举值（与管理端保持一致）：

| status | 展示阶段（客户端） | 含义 | 客户端交互约束 |
|---|---|---|---|
| DRAFT | 筹备中 | 未发布/不可报名 | 禁止报名/取消报名；不展示排名 |
| PUBLISHED | 报名中 | 发布后开放报名 | 允许报名/取消报名；不展示排名（可展示报名名单） |
| RUNNING（可选） | 进行中 | 比赛进行中（报名关闭） | 禁止报名/取消报名；不展示排名（可展示报名名单） |
| FINISHED | 已结束 | 比赛结束 | 禁止报名/取消报名；允许查看排名/成绩 |
| CANCELED | 已取消 | 赛事被取消 | 禁止报名/取消报名；不展示排名 |

### tournament-phase（客户端计算）
如果后端暂时只有 `DRAFT/PUBLISHED/FINISHED/CANCELED`，也可由客户端通过时间推导“阶段”：

- 当 `status=CANCELED`：已取消
- 当 `status=FINISHED`：已结束
- 当 `status=DRAFT`：筹备中
- 当 `status=PUBLISHED` 且 `now < startAt`：报名中
- 当 `status=PUBLISHED` 且 `startAt <= now < endAt`：进行中（报名停止）
- 当 `status=PUBLISHED` 且 `now >= endAt`：已结束（建议后端最终把 status 更新为 FINISHED）
...
- 报名中/进行中：`items` 应为空数组（或不返回 `items`），且不返回 `my`
- 已结束：返回完整 `items`，并可选返回 `my`
